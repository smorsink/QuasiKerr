...
/****************************************************************************
 * surfacefit finds the parameters that fit a curve to the legendre 
 *      polynomials that fit the surface of the star
 *
 * Source code for SetUpStar, MakeSphere, and Kepler can be found in findmodel.c
 * Source code for Surface and LegnFit can be found in surface.c

 ****************************************************************************
 * SurfPrint prints out stuff found in surface
 ***************************************************************************/
...
int SurfPrint(EOS *eos, NeutronStar *star){
  double
    ...
    *r_BL_surf,           /*Surface in Boyer-Lindquist coordinates */
    enthalpy_min;    /*Minimum Enthalpy*/
  int i;
  char filename[80]="surface.txt";
  FILE *fp;
  ...
  fprintf(fp, "#Surface Data\n");
  fprintf(fp, "#Column 1: mu = cos(theta)\n");
  fprintf(fp, "#Column 2: s coordinate at the star's surface (see CST)\n");
  fprintf(fp, "#Column 3: r_iso = isotropic radial coordinate in units of the star's mass\n");
  fprintf(fp, "#Column 4: r = Schwarzschild radial coordinate in units of the star's mass\n");
  fprintf(fp, "#Column 5: r_BL = Boyer-Linquist radial coordinate in units of the star's mass\n");



  for(i=1; i<=MDIV; i++){  
    fprintf(fp, "%6.5e\t%6.5e\t%6.5e\t%6.5e\t%6.5e\n", mu[i], s_surf[i], r_is_surf[i], r_surf[i], r_BL_surf[i]);
  }
  fclose(fp);

  printf("\nInformation about the Star's surface has been sent to file: %s \n\n", filename);
...
int Surface( EOS *eos, NeutronStar *star){
  ...
  double 
    ...
    *r_is_surf,      /*Surface, with isotropic r*/
    *r_surf,         /*Surface, with Schwartzchild r*/
    *r_BL_surf,      /*Surface in Boyer Lindquist */
  ...
  // mass is the dimensionless mass
   ...
   // printf("Dimensionless mass = %g \n", mass);
   //printf("Dimensionless radius= %g \n", r_e);

   // a_kerr is the dimensionless "a" parameter in the kerr metric

  a_kerr = star->j * mass;

  //printf(" Kerr parameter a = %g \n", a_kerr);
  ...
  for(i=1; i<= MDIV; i++){
	...
	// Convert radial coordinates to "mass" units by dividing by the dimensionless mass.
	r_surf[i] = r * exp((gama-rho)*0.5)/mass;
	r_BL_surf[i] = r * ( 1.0 + 0.5*(mass+a_kerr)/r) * ( 1.0 + 0.5*(mass-a_kerr)/r)/mass;
	r_is_surf[i] *= 1.0/mass;
