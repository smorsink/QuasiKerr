...
/*  Returns values of M, R, J, Omega, in cgs unit                      */
/***********************************************************************/
void mass_radius(
		 double s_gp[SDIV+1],
		 double mu[MDIV+1],
  ...
  while(dif> accuracy || n_of_it<2) { 
      ...
      */        
      ...
      for(s=1;s<=SDIV;s++)
	    ...
	    // These are the "Tilde" potentials, multiplied by r^2 
      ...
      // Compute Quadrupole Moment (via Laarakkers and Poisson)
      ...
      for(k=1;k<=SDIV-3;k+=2) { 
	(*quadrupole) += (DS/3.0)*( pow( s_gp[k]/(1.0-s_gp[k]), 2.0*n) / pow( (1.0-s_gp[k]),2) *D1_rho[n+1][k] 
			      + 4.0*pow( s_gp[k+1]/(1.0-s_gp[k+1]), 2.0*n) / pow( (1.0-s_gp[k+1]),2) *D1_rho[n+1][k+1]
			      + pow( s_gp[k+2]/(1.0-s_gp[k+2]), 2.0*n) / pow( (1.0-s_gp[k+2]),2)   *D1_rho[n+1][k+2]);   

	(*Btilde) += (DS/3.0)*( s_gp[k] / pow( (1.0-s_gp[k]),3)*D1_gama[n+1][k] 
                           + 4.0*s_gp[k+1] / pow( (1.0-s_gp[k+1]),3)*D1_gama[n+1][k+1]
                           + s_gp[k+2] / pow( (1.0-s_gp[k+2]),3)*D1_gama[n+1][k+2]);
	...
	(*Btilde) += (DS/3.0)*( s_gp[k] / pow( (1.0-s_gp[k]),3)*D1_gama[n+1][k] 
      ...
      (*quadrupole) *= 0.5 * pow(r_e,3) * pow(KAPPA,1.5) * SQ(C) / G;  // Quadrupole moment in units of g cm^2

      (*Btilde) *= -2.0/PI ; // B_0 tilde in units of g cm^3/s^2 

      //printf("Old mass quadrupole = %g \n",
      //  0.5*pow(r_e*s_gp[SDIV-1]/(1.0-s_gp[SDIV-1]),3)*D2_rho[SDIV-1][2] * pow(sqrt(KAPPA),3)* C*C/(G));

      //printf("New mass quadrupole = %g \n",
      //	     (*quadrupole));


      // printf("Btilde = %g  \n", *Btilde);

      //printf("Btilde/r^2 = %g \n", *Btilde * pow( (1.0 - s_gp[SDIV-1])/(s_gp[SDIV-1]),2) );
 
      //printf(" gamma1 = %g \n", -2.0/PI * D2_gama[SDIV-1][2] );
