	...
	NeutronStar *star)
{
  ...
  spin(star->metric.s_gp, star->metric.mu, 
       ...
       a_check, accuracy, cf,
       star->r_ratio, &star->r_e, &star->Omega, &star->Quad, &star->Btilde);
  ...
  for (s=1;s<=SDIV;s++){
      ...
      star->metric.alpha_mu[s][m] = 0;
      star->metric.alpha_ms[s][m] = 0;

      star->metric.rho_s[s][m] = deriv_s(star->metric.rho,s,m);
      star->metric.rho_mu[s][m] = 0;
      star->metric.rho_ms[s][m] = 0;

      star->metric.gama_s[s][m] = deriv_s(star->metric.gama,s,m);
      star->metric.gama_mu[s][m] = 0;
      star->metric.gama_ms[s][m] = 0;

      star->metric.omega_s[s][m] = deriv_s(star->metric.omega,s,m);
      star->metric.omega_mu[s][m] = 0;
      star->metric.omega_ms[s][m] = 0;
    }
	     ...
	     double e_center, double spinfreq)
{
  ...
  double ans, fh,fl,fm,fnew,sroot,xh,xl,xm,xnew,xacc=1e-5;
...
int MaxMass(double e_min, double e_max, EOS *eos, NeutronStar *star, char name[80])
{
  ...
  for (i=0;i<=n;i++){
    ...
    fprintf(output,"%g %g %g %g %g %g %g %g %g\n",
	    e_center, star->Mass/MSUN, star->Mass_0/MSUN,
	    star->R_e*1e-5, xx,
	    sqrt(star->Mass*G/pow(star->R_e,3)),
	    sqrt(1.0-2.0*xx),
	    1.0/sqrt(1.0-2.0*xx)-1.0,
	    star->Mass_0/star->Mass - 1.0);
  ...
  if (flag==1){
    ...
    */
    ...
    fprintf(output,"%g %g %g %g %g %g %g %g %g\n",
	    e_max, star->Mass/MSUN, star->Mass_0/MSUN,
	    star->R_e*1e-5, xx,
	    sqrt(star->Mass*G/pow(star->R_e,3)),
	    sqrt(1.0-2.0*xx),
	    1.0/sqrt(1.0-2.0*xx)-1.0,
